...
param = get_parameters(36, 36)
P["preterm_fac"] = param["preterm_fac_target"]
...
def f_ANC_LB_effect(PT_scale):
    ...
    # Predicted outcomes
    P_L45 = np.sum(n["LB_L"][2:]) / np.sum(n["LB_L"])
    Preterm_fac = np.sum(M["PT"][1:]) / np.sum(n["LB_L"][1:])

    return Preterm_fac, P_L45
...
Preterm_fac = np.mean(results[0])
P_L45 = np.mean(results[1])
print("Predicted outcomes:", Preterm_fac, P_L45)  # Debug print
