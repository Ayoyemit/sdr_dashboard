...
def get_slider_params():
    slider_params = {
        ...
        'base_knowledge_L45_slider': 0.60,
        'base_p_45_slider': (20709 + 5126) / np.sum(np.array([23729, 18196, 20709, 5126])),
        'p_ANC_base_slider': 0.56,
        'S_pph_bundle_slider': np.array([0, 0, 0, 0]),
        'S_iv_iron_slider': 0.44,
        'S_MgSO4_slider': np.array([0, 0, 0.77, 0.77]),
        'S_antibiotics_slider': np.array([0, 0, 0.48, 0.48]),
        'S_oxytocin_slider': np.array([0, 0, 0.78, 0.78]),
        't_l23_l45_notsevere_slider': 76,
    }
...
def get_parameters(rng = None):
    ...
    param = {
        ...
        ##mothers' conditions##
        'p_highrisk': sample_from_ci(0.26, 0.247, 0.273, 4419, 'proportion', 1, rng = rng)[0], #0.26,                  # probability of high risk pregnancy
        'p_ANC_base': sample_from_ci(0.56, 0.494, 0.626, 216, 'proportion', 1, rng = rng)[0], #0.56,                   # probability of 4+ ANCs
        'class': sample_from_ci(0.572, 0.506, 0.638, 216, 'proportion', 1, rng = rng)[0],#0.572,                       # calculated as if above 3 for wealth, education, and has at least a motor bike or car, then at least 1 of the following
        'close_to_L23': sample_from_ci(0.89, 0.886, 0.894, 19127, 'proportion', 1, rng = rng)[0], #0.89,               # probability of mothers having cloest distance to L2/3 facility
        ...
        # calibrated
        "home_noANC": 0.7056735167353498,                                                                   # probability of home delivery without ANC
        "l45_fac": 0.21039299793502853,                                                                     # probability of delivery at L4/5 facility if chosing deliver at facilities and without ANC
        "home_lowrisk": 0.17051451153731043,                                                                # probability of home delivery if predicted as low risk
        "L23_highrisk": 0.36751767996993084,                                                                # probability of delivery at L2/3 facility if predicted as high risk
        "sen_risk_trad": 0.7939486174518111,                                                                # sensitivity of traditional ANC monitoring in predicting high risk
        "spec_risk_trad": 0.6311811663318615,                                                               # specificity of traditional ANC monitoring in predicting low risk
        "PT_scale": 0.8250540888309176,                                                                     # scaling factor for reducing preterm birth rate to Kenya level
        "p_elec_CS|highrisk": 0.0642, #0.2, #0.23741230625326665, # (BEFORE),  0.21,                                        # probability of elective CS if predicted as high risk
        "p_elec_CS|preterm": 0.7799,                                                                           # probability of elective CS if predicted as low risk
        "p_elec_CS|highrisk_us": 0.35,                                                                      # probability of elective CS if predicted as high risk by ultrasound - need calibration again
        "p_elec_CS|preterm_us": 0.7799,                                                                       # probability of elective CS if predicted as preterm by ultrasound - need calibration again
        # known parameters
        "p_lb_l23_45": sample_from_ci(0.122, 0.119, 0.125, n=50981, kind='proportion', size=1, rng = rng)[0], #0.122,  # probability of live births at L4/5 if incoming maternal referrals
        "GA_sequence": np.arange(27, 45),                                                                   # gestational age sequence
        'OR_preterm_ANC': sample_from_ci(0.48, 0.479, 0.481, kind='OR', size=1, rng = rng)[0], #0.48,                  # odds ratio of preterm birth given adequate ANC
        ...
        "E_GA_US": np.array([.057, .729]),                      #% Normal distribution parameters for GA error | ultrasound
        #"E_Preterm_LMP": np.array([0.643, 0.039, 0.019]),       #% P[Preterm_class|PT], P[Preterm_class|at term], P[Preterm_class|post term]
        'E_Preterm_LMP': np.array([
        ...
        #"E_Posterm_LMP": np.array([0.03, 0.105, 0.336]),        #% P[Posterm_class|PT], P[Posterm_class|at term], P[Posterm_class|post term]
        'E_Postterm_LMP': np.array([
       ...
       0.05834325, 0.02872319, 0.01586185]),
        'GA_noanc': np.array([0.00528652, 0.00299282, 0.0040846, 0.00607662, 0.00817399,
                           ...
                           0.0520695, 0.02563454, 0.0141562]),

        ##parameters in intrapatum.py##
        # calibrated
        "p_aph": 0.0176, #0.0179, #0.0172,                                        # probability of antepartum hemorrhage
        "p_eclampsia": 0.0164, #0.0159, #0.017,                                   # probability of eclampsia - calibrated to match the rate
        'p_ruptured_uterus': 0.0191, #0.0197, #0.019,                             # probability of getting other complications - calibrated to match the rate
        "p_cs_capacity": np.array([0, 0.0568, 0.1215, 0.1215]), #np.array([0, 0.05, 0.13, 0.13]), #np.array([0, 0.05, 0.15, 0.15])(BEFORE)      # probability of EmCS if predicted with complications -reflecting the capacity of CS - calibrated to match CS rate
        "p_OL_scale": 0.7982, #0.83,  #0.95(BEFORE)                                   # scaling factor for reducing obstructed labor from Ethiopia to Kenya level
        "p_pph_other": 0.0100, #0.014, #0.02(BEFORE) ,                                    # probability of PPH by not OL and not CS, but other risk factors - calibrated to match PPH in Kenya
        "p_mat_sepsis_other": 0.0356, #0.04, #0.0185(BEFORE) ,                           # probability of maternal sepsis by not OL, not CS, but other risk factors - calibrated to match matnernal sepsis in Kenya
        # assumption
        "p_cs_capacity_sdr": np.array([0, 0, 0.1215, 0.1215]), #np.array([0, 0, 0.15, 0.15])(BEFORE)      # no surgical capacity for L2/3 - assumption
        ...
        # known parameters
        'RR_comp_highrisk_vs_lowrisk': sample_from_ci(4.2, 1.79, 10.86, kind='RR', size=1, rng = rng)[0], #4.2,             # relative risk of complications if high risk compared to low-risk pregancies
        'p_PL_GA': np.array([
        ...
        "p_hypoxia": sample_from_ci(0.105, 0.083, 0.127, n=779, kind='proportion', size=1, rng = rng)[0], #0.105, #0.098,                     # probability of hypoxia
        "p_mat_sepsis_OL": sample_from_ci(0.357, 0.227, 0.474, n=None, kind='proportion', size=1, rng = rng)[0], #0.357,                       # probability of maternal sepsis by OL
        "p_mat_sepsis_elective_CS": sample_from_ci(0.07, 0.032, 0.108, n=170, kind='proportion', size=1, rng = rng)[0], #0.07,               # probability of maternal sepsis by elective CS
        "p_mat_sepsis_emergency_CS": sample_from_ci(0.335, 0.264, 0.406, n=170, kind='proportion', size=1, rng = rng)[0], #0.335,             # probability of maternal sepsis by emergency CS
        "p_pph_OL": sample_from_ci(0.148, 0.088, 0.208, n=135, kind='proportion', size=1, rng = rng)[0], #0.148,                              # probability of PPH by OL
        "p_pph_elective_CS": sample_from_ci(0.064, 0.027, 0.101, n=170, kind='proportion', size=1, rng = rng)[0], #0.064,                     # probability of PPH by elective CS
        "p_pph_emergency_CS": sample_from_ci(0.188, 0.129, 0.247, n=170, kind='proportion', size=1, rng = rng)[0], #0.188,                    # probability of PPH by emergency CS
        "p_stillbirth_OL": sample_from_ci(0.185, 0.119, 0.251, n=135, kind='proportion', size=1, rng = rng)[0], #0.185,                       # probability of stillbirth by OL
        "p_asphyxia_OL": sample_from_ci(0.692, 0.515, 0.869, n=26, kind='proportion', size=1, rng = rng)[0], #0.692,                         # probability of asphyxia by OL
        "p_neo_sepsis_OL": sample_from_ci(0.165, 0.102, 0.228, n=135, kind='proportion', size=1, rng = rng)[0], #0.165,                       # probability of neonatal sepsis by OL
        ...
        'sen_prolonged_IS': sample_from_ci(0.74, 0.65, 0.82, n=None, kind='proportion', size=1, rng = rng)[0], #0.74,                                # sensitivity of predicting prolonged labor by intrapartum sensors
        'spec_prolonged_IS': sample_from_ci(0.77, 0.69, 0.84, n=None, kind='proportion', size=1, rng = rng)[0], # 0.77,                      # specificity of predicting prolonged labor by intrapartum sensors
        'sen_ol_IS': sample_from_ci(0.87, 0.817, 0.923, n=154, kind='proportion', size=1, rng = rng)[0],# 0.87,                              # sensitivity of predicting obstructed labor by intrapartum sensors
        "spec_ol_IS": 0.95,                             # specificity of predicting obstructed labor by intrapartum sensors
        'sen_hypoxia_IS': sample_from_ci(0.774, 0.739, 0.809, n=552, kind='proportion', size=1, rng = rng)[0],# 0.774,                        # sensitivity of predicting hypoxia by intrapartum sensors
        'spec_hypoxia_IS': sample_from_ci(0.939, 0.919, 0.959, n=552, kind='proportion', size=1, rng = rng)[0],#0.939,                       # specificity of predicting hypoxia by intrapartum sensors
        'CS_AVD_ratio': sample_from_ci(0.985, 0.984, 0.986, n=44031, kind='proportion', size=1, rng = rng)[0],
        #"CS_AVD_ratio": [0.985, 0.015],                 # ratio of CS to AVD
        'p_comp_anemia': sample_from_ci(0.25, 0.250, 0.250, n=3544672, kind='proportion', size=1, rng = rng)[0], # 0.25,                                  # probability of getting anemia
        'or_anc_anemia': sample_from_ci(2.26, 1.05, 4.89, kind='OR', size=1, rng = rng)[0], # 2.26,                                  # odds ratio of getting anemia without ANC
        'scale_comps': 4.48,                            # scaling factor for increasing maternal complications to Kenya level
        'or_anemia_pph': sample_from_ci(3.54, 1.20, 10.4, kind='OR', size=1, rng = rng)[0], #3.54,                          # odds ratio of getting PPH given anemia
        'or_anemia_sepsis': sample_from_ci(5.68, 4.38, 7.36, kind='OR', size=1, rng = rng)[0], #5.68,                       # odds ratio of getting sepsis given anemia
        'or_anemia_eclampsia': sample_from_ci(3.74, 2.79, 4.81, kind='OR', size=1, rng = rng)[0], #3.74,                    # odds ratio of getting eclampsia given anemia
        "p_RDS_noT": np.array([90] + [60] * 4 + [40] * 3 + [20] * 2 + [0] * 8) / 100,       # probability of RDS by GA without treatment
        'RR_comp_severe_highrisk_vs_lowrisk': sample_from_ci(4.2, 1.68, 10.5, kind='RR', size=1, rng = rng)[0],  #4.2,      # relative risk of severe complications for high-risk mothers compared to low-risk mothers

        ##parameters in mortality.py##
        # calibrated
        "p_MM_others": 0.0053, #30.8/10000,
        "MM_weight_pph": 2.0000, #1.92,
        "MM_weight_sepsis": 0.4760, #0.515,
        "MM_weight_eclampsia": 2.0000, #0.70,
        "MM_weight_ol": 0.1855, #0.32,
        #"MM_weight_ruptured_uterus":0.3,
        "MM_weight_aph": 2.0000, #1.176,
        'p_MM_home': 0.1456, #0.2, #0.252, #0.376, #0.295 (BEFORE),     (0.1, 0.3)                            # maternal mortality rate due to SMO by home - calibrated to match MMR at home
        'weight_facility_mat': 2.6661, #5, #4.21, #8.5, #11.5(BEFORE),  (1-5)                        # weight of facility in calculating MMR due to healthcare worker density if getting SMO
        'p_comp_severe_lowrisk': 0.0501, #0.041,                                             # probability of severe if complications for low-risk mothers - calibrated to match severe maternal outcome rate at L4/5
        'p_NM_home': 0.235,
        'weight_facility_neo': 3.15,
        # known parameters
        'OR_MM_CSvsSVD': sample_from_ci(2.28, 1.87, 2.79, kind='OR', size=1, rng = rng)[0],# 2.28,                            # odds ratio of maternal mortality by CS vs SVD
        'OR_MM_EmCSvsELCS': sample_from_ci(3.17, 2.48, 4.04, kind='OR', size=1, rng = rng)[0],#3.17,                         # odds ratio of maternal mortality by emergency CS vs elective CS
        'OR_MM_transfer': sample_from_ci(1.59, 1.30, 1.93, kind='OR', size=1, rng = rng)[0], #1.59,                           # odds ratio of maternal mortality by transfer
        'OR_NM_transfer': sample_from_ci(2.5, 1.1, 5.6, kind='OR', size=1, rng = rng)[0], #2.5,                             # odds ratio of neonatal mortality by transfer
        ...
        # calibrated
        't_l23_l45_preterm': 80.9485, #81,                         # probability of transfer from L2/3 to L4/5 if preterm - calibrated to match preterm rate by facility level
        't_l23_l45_notsevere': 74.2358, #76,                       # probability of transfer from L2/3 to L4/5 if not severe - calibrate to match live birth distribution by facility level
        't_l23_l45_severe': 90.0000, #80,                          # probability of transfer from L2/3 to L4 - calibrated to match severe maternal outcome rate at L4/5
        # known parameters
        ...
        #ANC
        'S_iv_iron': 0.44,  # 0.075(OLD),                             # the supply level of IV iron at facilities
        #Intrapartum + PNC
        'S_oxytocin_l45': sample_from_ci(0.78, 0.706, 0.854, n=120, kind='proportion', size=1, rng = rng)[0],
        #"S_oxytocin": np.array([0, 0, 0.78, 0.78]),     #np.array([0, 0.43, 1, 1])(OLD),    # coverage of oxytocin by facility levels - 0.43 was average pharms at L2/3
        'S_preterm_treat_l45': sample_from_ci(0.35, 0.285, 0.415, n=206, kind='proportion', size=1, rng = rng)[0],
        #"S_preterm_treat": np.array([0, 0, 0.35, 0.35]), #np.array([0, 0.43, 1, 1])(OLD),   # coverage of preterm treatment by facility levels
        'S_pph_bundle': np.array([0, 0, 0, 0]),                                      # the supply level of obstetric drape at L4/5
        'S_MgSO4': np.array([0, 0, 0.77, 0.77]), #0.470(OLD),                               # the supply level of MgSO4 at L4/5
        'S_antibiotics': np.array([0, 0, 0.48, 0.48]),#0.530(OLD),                         # the supply level of antibiotics at L4/5
        ...
        'OR_NEC_treat': sample_from_ci(0.28, 0.14, 0.56, n=580, kind='OR', size=1, rng = rng)[0], #0.28,                           # odds ratio of preterm having NEC given treatment
        'RR_Sepsis_treat': sample_from_ci(0.24, 0.13, 0.44, n=2063, kind='RR', size=1, rng = rng)[0], #0.24,                        # relative risk of preterm having sepsis given treatment
        'E_pph_bundle': sample_from_ci(0.51, 0.44, 0.60, kind='RR', size=1, rng = rng)[0], #0.9, #0.51, #0.40 (OLD),               # efficacy of obstetric drape in reducing PPH
        'E_iv_iron': sample_from_ci(0.30, 0.203, 0.397, n=86, kind='proportion', size=1, rng = rng)[0], # 0.30,                              # efficacy of IV iron infusion in reducing anemia
        'E_MgSO4': sample_from_ci(0.41, 0.29, 0.58, n=11444, kind='RR', size=1, rng = rng)[0], #0.41,                                # efficacy of MGSO4 in reducing eclampsia
        'E_antibiotics': sample_from_ci(0.67, 0.56, 0.79, n=14590, kind='RR', size=1, rng = rng)[0], #0.67, #0.20,                   # efficacy of antibiotics in reducing maternal sepsis
        'E_oxytocin': sample_from_ci(0.91, 0.827, 0.993, n=46, kind='proportion', size=1, rng = rng)[0], #0.91,                             # efficacy of oxytocin

        ##parameters in DALY calculation##
        'DW': {
            'anemia': sample_from_ci(0.052, 0.0338, 0.0757, kind='mean', size=1, rng = rng)[0], #0.052,
            'low pph': sample_from_ci(0.114, 0.0779, 0.1587, kind='mean', size=1, rng = rng)[0],# 0.114,
            'high pph': sample_from_ci(0.324, 0.2197, 0.4418, kind='mean', size=1, rng = rng)[0], #0.324,
            'maternal sepsis': sample_from_ci(0.133, 0.0884, 0.1895, kind='mean', size=1, rng = rng)[0], #0.133,
            'eclampsia': sample_from_ci(0.602, 0.4266, 0.7527, kind='mean', size=1, rng = rng)[0], #0.602,
            'obstructed labor': sample_from_ci(0.324, 0.2197, 0.4418, kind='mean', size=1, rng = rng)[0], #0.324,
            'maternal death': 1,
            'neonatal death': 1,
            'preterm comp': 0.388,
            'asphyxia': 0.549,
        ...
        'Mother_life_expectancy': 69.2,
        'Neonate_life_expectancy': 66.8,
        'Childbearing_age': 28.6,
...
def calculate_derived_parameters(param):
    ...
    param["severe_highrisk"] = param['p_comp_severe_lowrisk'] * param[
        'RR_comp_severe_highrisk_vs_lowrisk']  # probability of severe complications for high-risk pregnancies
    param["severe"] = np.array(
        [param['p_comp_severe_lowrisk'], param["severe_highrisk"]])  # probability of severe complications by risk level
    param["OL"] = param["p_OL"] * param["p_OL_scale"]  # probability of obstructed labor if not prolonged vs prolonged

    param["OL_highrisk"], param["OL_lowrisk"] = comps_riskstatus_vs_lowrisk(param["OL"][0], param['p_highrisk'],
                                                                            param["RR_comp_highrisk_vs_lowrisk"])
    ...
    # supply
    param["S_oxytocin"] = np.array([0, 0, param["S_oxytocin_l45"], param["S_oxytocin_l45"]])
